...
def plot_compression_comparison():
    ...
    ax2.plot(
        flow_df["k"],
        flow_df["test_mse_FlowPCA"],
        "^-",
        label="Flow→PCA (RealNVP)",
        color="#F18F01",
        linewidth=2.5,
    )
...
def plot_delta_comparison():
    ...
    ax.set_title(
        "Improvement Over Standard PCA (negative = better)", fontweight="bold", pad=15
    )
    ax.set_xticks(x)
...
def plot_memory_reduction():
    ...
    bars = ax1.bar(
        configs, memory, color=colors, alpha=0.8, edgecolor="black", linewidth=1.5
    )
    ...
    bars2 = ax2.bar(
        configs, reduction, color=colors, alpha=0.8, edgecolor="black", linewidth=1.5
    )
    ...
    ax2.axhline(
        y=90, color="red", linestyle="--", linewidth=2, alpha=0.6, label="90% target"
    )
...
def plot_architecture_diagram():
    ...
    ax.text(
        6,
        3.7,
        "KV-Geometry Compression Pipeline",
        fontsize=14,
        fontweight="bold",
        ha="center",
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "architecture_diagram.png", dpi=300, bbox_inches="tight", facecolor="white"
    )
